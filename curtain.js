...
local.createElements = function()
    {
        ...
        { 
          ...
          var g = 255;
          var b = 255;
          var randCol = 'rgba('+r+','+g+','+b+','+(Math.random()*0.2+0.1)+')';//'#'+Math.floor(Math.random()*16777215).toString(16);
...
local.createRectangle = function(i)
{
    ...
    elem.setAttribute("width",Math.random()*5+2000);
...
local.moveElement = function(elem,i)
{
  ...
  console.log(xDiff);
  TweenMax.to(elem,Math.random()*6+1,{attr:{width:Math.random()*500+100},
                                       x:xDest,
                                       rotationZ:xDiff/50,
